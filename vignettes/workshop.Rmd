...
date: "Last modified: November 20, 2019; Compiled: `r format(Sys.time(), '%B %d, %Y')`"
output: 
  html_document:
    toc: true
    toc_float: true
    ...
    code_folding: hide
vignette: >
  %\VignetteIndexEntry{Building a Bioconductor Package}
...
There are 1,903 software packages on Bioconductor packages `r emo::ji("package")` currently available. 
...
with R" which is available for free [online](https://happygitwithr.com/).
...
#### Github actions `r emo::ji("cat")`

Github actions automatically allow you to check your package every time you commit a change. This will allow you to catch errors quickly. However, they can be tricky to set up. Good that there is a package that will do the hard work for you, simply add Github actions  with the help of the `biocthis` `r emo::ji("package")`. Check out its function `use_bioc_github_action()` for more information.
...
way to get started is to use `usethis::use_readme_rmd()`. This creates a template 
...
Also check out the amazing [`biocthis`](https://github.com/lcolladotor/biocthis/) package which will guide you through the development of a package when this tutorial fails.
